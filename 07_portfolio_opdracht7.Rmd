...
the new tidy data files were exporting to DBeaver using Rpostgres: 
...
After inspected the data in Dbeaver, the data was also still inspected in R using the dplyr package. Based on these 2 checks, it was concluded that the data was loaded in properly. 

```{r 07 dyplr table, include=TRUE, warning = FALSE, message = FALSE, echo = FALSE}
dplyr::data_frame(dengue_tidy)
```
